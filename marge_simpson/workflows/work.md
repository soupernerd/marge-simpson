...
```powershell
Select-String -Path ".marge/knowledge/decisions.md" -Pattern "#auth|#api|#database"
```
...
1. Create the next MS-#### ID
2. Add entry to `.marge/assessment.md`:
   ```markdown
   ...
   ```
3. Add task to `.marge/tasklist.md`:
   ```markdown
   ### [MS-####] Short description
   - **Type:** bug | feature | improvement
   - **DoD:** What "done" looks like
   - **Verification:** `./.marge/scripts/verify.ps1 fast`
   - **Status:** [ ] Not started
   ```
...
┌─────────────────────────────────────┐
│  2. VERIFY                          │
│     - Run: ./.marge/scripts/verify.ps1 fast │
